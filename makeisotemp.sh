#!/usr/bin/env sh
cd ~/git/porteus
#find | cpio -H newc -o | xz --threads=8 --check=crc32 --x86 --lzma2 > ~/git/makeiso/linux/initrd.xz
cd ~/git/makeiso
...
xorriso -as mkisofs -R -hide-joliet boot.catalog -b fmldr -no-emul-boot -allow-lowercase -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e efi.img -no-emul-boot -o porteus_multiarch.iso build
